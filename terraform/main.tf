locals {
  ...
  gitops_addons_url      = var.gitops_addons_repo_url != "" ? var.gitops_addons_repo_url : "${var.gitops_addons_org}/${var.gitops_addons_repo}"
...
module "aks" {
  ...
  vnet_subnet_id                                  = azurerm_subnet.spoke_aks["mgmt-we"].id
  os_disk_size_gb                                 = var.os_disk_size_gb
  os_sku                                          = var.os_sku
  sku_tier                                        = var.sku_tier
  private_cluster_enabled                         = var.private_cluster_enabled
  identity_type                                   = "UserAssigned"
  identity_ids                                    = [azurerm_user_assigned_identity.mgmt_cluster["mgmt-we"].id]
  ...
  node_pools = {
    cipool = {
      ...
      node_taints     = ["workload=ci:NoSchedule"]
      os_disk_size_gb = var.os_disk_size_gb
      os_sku          = var.os_sku
  ...
  network_contributor_role_assigned_subnet_ids = { "aks" = azurerm_subnet.spoke_aks["mgmt-we"].id }
...
resource "kubernetes_secret" "git_secrets" {
  ...
  for_each = {
    git-addons = merge(
      ...
      fileexists(pathexpand(var.git_private_ssh_key)) ? {
...
module "gitops_bridge_bootstrap" {
  ...
  cluster = {
    ...
    addons = merge(local.addons, {
      ...
      "role"                           = "management"
      "lease-status"                   = "active"
      "akuity.io/argo-cd-cluster-name" = module.aks.aks_name
    })
  }
