...
| **FR-V3-05** — TLS history purge incomplete: `git filter-repo` rewrite was never performed; `BEGIN PRIVATE KEY` blobs remain reachable from `7b07f95` (original commit) and `9d04b71` (the `git rm` commit, which is not a history rewrite) on the canonical remote | **Resolution chosen 2026-05-26: full remediation** via destructive history rewrite + force-push + cert re-revocation + re-clone runbook. Captured as `FR-V3.1-06..09` and `US-V3.1-02` in PRD-v3.1. **Runbook + mirror-clone purge + verifier scripts landed 2026-05-26** (`docs/tls-history-purge-2026-05-runbook.md`, `scripts/tls-history-purge-mirror.sh`, `scripts/verify-tls-history-clean.sh`); live cutover scheduled with platform-security lead. OQ-V4-07 is closed by this decision | `_docs/IDP-GitOps-Blueprint-PRD-v3.1.md` US-V3.1-02 |
